...
chess_board::chess_board() {//constructor
    matrix_data = new std::unique_ptr<chess_pieces>[64]; // unique pointer of chess pieces
    // Placing chess pieces in correct start position
    matrix_data[index(1, 1)] = std::make_unique<rook>("w");
    matrix_data[index(1, 8)] = std::make_unique<rook>("w");
    matrix_data[index(1, 2)] = std::make_unique<knight>("w");
    matrix_data[index(1, 7)] = std::make_unique<knight>("w");
    matrix_data[index(1, 3)] = std::make_unique<bishop>("w");
    matrix_data[index(1, 6)] = std::make_unique<bishop>("w");
    matrix_data[index(1, 4)] = std::make_unique<king>("w");
    matrix_data[index(1, 5)] = std::make_unique<queen>("w");
    matrix_data[index(8, 1)] = std::make_unique<rook>("b");
    matrix_data[index(8, 8)] = std::make_unique<rook>("b");
    matrix_data[index(8, 2)] = std::make_unique<knight>("b");
    matrix_data[index(8, 7)] = std::make_unique<knight>("b");
    matrix_data[index(8, 3)] = std::make_unique<bishop>("b");
    matrix_data[index(8, 6)] = std::make_unique<bishop>("b");
    matrix_data[index(8, 4)] = std::make_unique<king>("b");
    matrix_data[index(8, 5)] = std::make_unique<queen>("b");
    for (int i = 1; i <= 8; i++) { // for loop to adding multiple pawns and empty pieces
        matrix_data[index(7, i)] = std::make_unique<pawn>("b");
        matrix_data[index(2, i)] = std::make_unique<pawn>("w");
        for (int j = 3; j <= 6; j++) {
            matrix_data[index(j, i)] = std::make_unique<empty>();
        }
    }
}
chess_board::chess_board(std::vector<std::pair<std::string, int>> board) {//parameterised constructor from load file info
    matrix_data = new std::unique_ptr<chess_pieces>[64]; //create a unique pointer array filled with empty pieces
    for (size_t i = 0; i < 64; i++) {
        matrix_data[i] = std::make_unique<empty>();
    }
    for (auto it = board.begin(); it != board.end(); it++) { // iterator over vector which contains piece and location. Replaces empty piece by given piece in a location
        if (it->first == "bp") { matrix_data[it->second] = std::make_unique < pawn>("b"); }
        else if (it->first == "wp") { matrix_data[it->second] = std::make_unique < pawn>("w"); }
        else if (it->first == "br") { matrix_data[it->second] = std::make_unique<rook>("b"); }
        else if (it->first == "wr") { matrix_data[it->second] = std::make_unique<rook>("w"); }
        else if (it->first == "bn") { matrix_data[it->second] = std::make_unique<knight>("b"); }
        else if (it->first == "wn") { matrix_data[it->second] = std::make_unique<knight>("w"); }
        else if (it->first == "bb") { matrix_data[it->second] = std::make_unique<bishop>("b"); }
        else if (it->first == "wb") { matrix_data[it->second] = std::make_unique<bishop>("w"); }
        else if (it->first == "bq") { matrix_data[it->second] = std::make_unique<queen>("b"); }
        else if (it->first == "wq") { matrix_data[it->second] = std::make_unique<queen>("w"); }
        else if (it->first == "bk") { matrix_data[it->second] = std::make_unique<king>("b"); }
        else if (it->first == "wk") { matrix_data[it->second] = std::make_unique<king>("w"); }
        else {}
    }
}
...
bool chess_board::is_empty(int board_x_pos, int board_y_pos) {// checks if there is an empty piece in a given location
    if (matrix_data[index(board_x_pos, board_y_pos)]->col() == " ") { return true; }
    else { return false; }
}
bool chess_board::right_colour(int board_x_pos, int board_y_pos, std::string colour) { // Compares piece colour and player colour : returns true if they are the same
    if ((matrix_data[index(board_x_pos, board_y_pos)]->col() == "w" && colour == "White") || (matrix_data[index(board_x_pos, board_y_pos)]->col() == "b" && colour == "Black")) {
        return true;
    }
    else { return false; }
}
...
std::vector<std::pair<int, int>>  chess_board::possible_moves(int board_x_pos, int board_y_pos) {
    ...
    move_list = matrix_data[index(board_x_pos, board_y_pos)]->move(); // Takes possible moves from move() function for the piece in the given location
    ...
    if (matrix_data[index(board_x_pos, board_y_pos)]->pic() == "p") { // checks if piece is a pawn: special case due to differences when taking pieces
            ...
            for (auto relative_positions : directions) {
                ...
                if (x > 8 || x < 1 || y > 8 || y < 1) { not_blocked = 0; } // checks if final position is out of the board
                    ...
                    if (directions == move_list[0]) { // checks if it is a non-taking move
                        if (matrix_data[index(x, y)]->pic() == " " && not_blocked == true) { //checks if position is empty and unblocked by piece in previous location
                            move.first = x;
                            move.second = y;
                    ...
                    else if (matrix_data[index(x, y)]->pic() != " " && matrix_data[index(board_x_pos, board_y_pos)]->col() != matrix_data[index(x, y)]->col()) { // for taking moves: checks if there is a piece of the opposite colour in taking positions
                        move.first = x;
                        move.second = y;
                        all_moves.push_back(move);
                    }
        ...
        for (auto directions : move_list) {
            ...
            for (auto relative_positions : directions) {
                if (not_blocked) {
                    int x = relative_positions.first + board_x_pos;
                    int y = relative_positions.second + board_y_pos;
                    if (x > 8 || x < 1 || y > 8 || y < 1 || matrix_data[index(board_x_pos, board_y_pos)]->col() == matrix_data[index(x, y)]->col()) { //does not save new position if it is off the board or a piece of the same colour
                        not_blocked = false; // indicates all subsequent moves are also not possible
                    }
                    else if (matrix_data[index(x, y)]->col() == " ") { // saves location if moving into location of empty piece
                        move.first = x;
                        move.second = y;
                        all_moves.push_back(move);
                    }
...
void chess_board::pawn_to_queen(int board_x_pos, int board_y_pos) { // function which changes a pawn to a queen if it reaches the opposite side of the board
    p_to_q = ""; // reset p_to_q
    if (matrix_data[index(board_x_pos, board_y_pos)]->piece_out() == "bp" && board_x_pos == 1) { // black pawn case
        matrix_data[index(board_x_pos, board_y_pos)] = std::make_unique<queen>("b");
        p_to_q = "Black pawn has changed into a Queen.\n";
        ;
    }
    else if (matrix_data[index(board_x_pos, board_y_pos)]->piece_out() == "wp" && board_x_pos == 8) {//white pawn case
        matrix_data[index(board_x_pos, board_y_pos)] = std::make_unique<queen>("w");
...
void chess_board::move_piece(int board_x_pos_start, int board_y_pos_start, int board_x_pos_end, int board_y_pos_end) {// function which moves piece
    if (matrix_data[index(board_x_pos_start, board_y_pos_start)]->pic() == "p") { matrix_data[index(board_x_pos_start, board_y_pos_start)]->has_moved(); } //if pawn is moved changes value of not_moved so it cant move 2 spaces from now on
    if (matrix_data[index(board_x_pos_end, board_y_pos_end)]->pic() == "k") { winner = matrix_data[index(board_x_pos_start, board_y_pos_start)]->col(); } //if king is taken returns which player has one into winner
    std::swap(matrix_data[index(board_x_pos_start, board_y_pos_start)], matrix_data[index(board_x_pos_end, board_y_pos_end)]); // swaps the pieces in the initial and end position
    matrix_data[index(board_x_pos_start, board_y_pos_start)] = std::make_unique<empty>(); // replaces the piece which was taken by an empty piece
...
std::ostream& operator<<(std::ostream& os, const chess_board& mat) { // overloaded << operator to output current state of board
    ...
    for (int i = 1; i <= 8; i++) {
        ...
        for (int j = 1; j <= 8; j++) {
            if (j == 8) { std::cout << mat.matrix_data[mat.index(i, j)]->piece_out() << end; } // outputs the piece in the given location
...
void chess_board::show_moves_out(int board_x_pos, int board_y_pos) { // function which outputs the chess board, representing the possible moves for a piece at the given location: similar to overloaded << operator ->differences stated
    ...
    for (int i = 1; i <= 8; i++) {
        ...
        for (int j = 1; j <= 8; j++) {
            if (j == 8) { std::cout << move_here_str(board_x_pos, board_y_pos, i, j) << matrix_data[index(i, j)]->piece_out() << move_here_str(board_x_pos, board_y_pos, i, j) << end; }
            else { std::cout << move_here_str(board_x_pos, board_y_pos, i, j) << matrix_data[index(i, j)]->piece_out() << move_here_str(board_x_pos, board_y_pos, i, j) << mid; }
        }
...
std::vector<std::pair<std::string, int>> chess_board::save_board() { // function to save locations of pieces and the type of piece
    ...
    for (size_t i = 0; i < 64; i++) {
        if (matrix_data[i]->piece_out() != "  ") { // checks where non empty pieces are located
